...
class PDTTrainer:
    ...
    def train_one_step(self, states, actions, returns, costs_return, time_steps, mask,
        ...
        qr_preds_current, qc_preds_current = self.model.pred_critics(sc, action_preds_mean)  # [batch_size, seq_len]

        # soft PF improvement
        N = 10

        # expand action_preds and sc for N samples
        sc = sc.unsqueeze(0).expand(N, -1, -1, -1)  # [N, batch_size, seq_len, state_dim]
        ...
        qr_preds, qc_preds = self.model.pred_critics(sc, q_actions)  # [N, batch_size, seq_len]
        ...
        beta = 2.0
        lag = qr_preds - lambd.unsqueeze(0) * qc_preds  # [N, batch_size, seq_len]
        lag = lag / beta
        weights = lag.exp()
        weights = weights / (weights.mean(dim=0, keepdim=True) + 1e-8)  # [N, batch_size, seq_len]
        ...
        loss_lag = (-(lambd * (qc_preds_current.detach() - costs_return)))[mask > 0]
