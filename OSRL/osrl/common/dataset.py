...
def set_labels(x: np.ndarray, gamma: float, seq_len: int = 20) -> np.ndarray:
    """
    Calculate the discounted cumulative sum of x (can be rewards or costs).
    """
    cumsum = np.zeros_like(x)
    for t in range(x.shape[0]):
        cumsum[t] = np.sum(x[t:t+seq_len])
    # cumsum[-1] = x[-1]
    # for t in reversed(range(x.shape[0] - 1)):
    #     cumsum[t] = x[t] + gamma * cumsum[t + 1]
    return cumsum
...
def set_cost_thresholds(dataset: dict):
    """
    Processes a given dataset for ccac.
    ...
    # get the indices of the transitions after terminal states or timeouts
    done_idx = np.where((dataset["terminals"] == 1) | (dataset["timeouts"] == 1))[0]
    dataset["cost_thresholds"] = np.zeros_like(dataset["costs"])
    dataset["next_cost_thresholds"] = np.zeros_like(dataset["costs"])
    ...
    for i in trange(done_idx.shape[0], desc="Processing trajectories"):
        ...
        cost_thresholds = discounted_cumsum(dataset["costs"][start:end], gamma=1.0)
        next_cost_thresholds = np.zeros_like(cost_thresholds)
        next_cost_thresholds[:-1] = cost_thresholds[1:]

        dataset["cost_thresholds"][start:end] = cost_thresholds
        dataset["next_cost_thresholds"][start:end] = next_cost_thresholds
...
class TransitionDataset(IterableDataset):
    ...
    def __prepare_sample(self, idx):
        ...
        if "cost_thresholds" in self.dataset.keys():
            cost_thresholds = self.dataset["cost_thresholds"][idx] * self.cost_scale
            next_cost_thresholds = self.dataset["next_cost_thresholds"][idx] * self.cost_scale
            return observations, next_observations, actions, rewards, costs, \
                cost_thresholds, next_cost_thresholds, done
